...
TEST(eof_validation, EOF1_undefined_opcodes)
{
    ...
    {
        // PUSH*, DUPN, SWAPN, RJUMP*, CALLF, JUMPF, CREATE3, RETRUNCONTRACT  require immediate
        ...
        if ((opcode >= OP_PUSH1 && opcode <= OP_PUSH32) || opcode == OP_DUPN ||
            ...
            opcode == OP_CREATE3 || opcode == OP_RETURNCONTRACT)
            continue;
...
TEST(eof_validation, EOF1_rjump_invalid_destination)
{
    ...
    auto cont =
        eof_bytecode(rjump(9) + 0 + 0xff + 0 + 0 + OP_CREATE3 + Opcode{0} + OP_POP + OP_STOP, 4)
            .container(embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_rjump_destination);

    // To RETURNCONTRACT immediate
    cont = eof_bytecode(rjump(5) + 0 + 0 + OP_RETURNCONTRACT + Opcode{0}, 2).container(embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_rjump_destination);
}
...
TEST(eof_validation, EOF1_rjumpi_invalid_destination)
{
    ...
    const auto embedded = eof_bytecode(bytecode{OP_INVALID});
    auto cont =
        eof_bytecode(rjumpi(9, 0) + 0 + 0xff + 0 + 0 + OP_CREATE3 + Opcode{0} + OP_POP + OP_STOP, 4)
            .container(embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_rjump_destination);
...
TEST(eof_validation, EOF1_rjumpv_invalid_destination)
{
    ...
    auto cont = eof_bytecode(
        rjumpv({9}, 0) + 0 + 0xff + 0 + 0 + OP_CREATE3 + Opcode{0} + OP_POP + OP_STOP, 4)
                    .container(embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_rjump_destination);
...
TEST(eof_validation, EOF1_section_order)
{
    ...
    //    // 01 02 03 04
    //    EXPECT_EQ(
    //        validate_eof("EF0001 010004 0200010006 0300010001 040002 00 00800001 6000E0000000 FE
    //        AABB"), EOFValidationError::success);
    //
    //    // 03 01 02 04
    //    EXPECT_EQ(
    //        validate_eof("EF0001 0300010001 010004 0200010006 040002 00 FE 00800001 6000E0000000
    //        AABB"), EOFValidationError::type_section_missing);
    //
    //    // 01 03 02 04
    //    EXPECT_EQ(
    //        validate_eof("EF0001 010004 0300010001 0200010006 040002 00 00800001 FE 6000E0000000
    //        AABB"), EOFValidationError::code_section_missing);
    //
    //    // 01 02 04 03
    //    EXPECT_EQ(
    //        validate_eof("EF0001 010004 0200010006 040002 0300010001 00 00800001 6000E0000000 AABB
...
{
    // no data section
    EXPECT_EQ(validate_eof("EF0001 010004 0200010006 0300010014 040000 00 00800001 6000E0000000 "
    ...
    EXPECT_EQ(validate_eof("EF0001 010004 0200010006 0300010006 040000 00 00800001 6000E0000000 "
                           "aabbccddeeff"),
        EOFValidationError::invalid_prefix);

    // multiple container sections
    EXPECT_EQ(
        validate_eof("EF0001 010004 0200010006 03000200140016 040000 00 00800001 6000E0000000 "
                     "EF000101000402000100010400000000800000FE "
...
TEST(eof_validation, EOF1_embedded_container_invalid)
{
    ...
    EXPECT_EQ(validate_eof("EF0001 010004 0200010006 030000 040000 00 00800001 60005D000000"),
        EOFValidationError::zero_section_size);

    // Container section with 0 size
    EXPECT_EQ(validate_eof("EF0001 010004 0200010006 0300010000 040000 00 00800001 60005D000000"),
        EOFValidationError::zero_section_size);

    // Container body missing
    EXPECT_EQ(validate_eof("EF0001 010004 0200010006 0300010014 040000 00 00800001 60005D000000"),
        EOFValidationError::invalid_section_bodies_size);
...
TEST(eof_validation, EOF1_create3_invalid)
{
    // truncated immediate
    const auto embedded = eof_bytecode(bytecode{OP_INVALID});
    auto cont = eof_bytecode(bytecode(0) + 0xff + 0 + 0 + OP_CREATE3, 4).container(embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::truncated_instruction);

    // last instruction
    cont = eof_bytecode(bytecode(0) + 0xff + 0 + 0 + OP_CREATE3 + Opcode{0}, 4).container(embedded);
    ...
    cont = eof_bytecode(bytecode(0) + 0xff + 0 + 0 + OP_CREATE3 + Opcode{1} + OP_POP + OP_STOP, 4)
               .container(embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_container_section_index);
    cont =
        eof_bytecode(bytecode(0) + 0xff + 0 + 0 + OP_CREATE3 + Opcode{0xff} + OP_POP + OP_STOP, 4)
            .container(embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_container_section_index);
}
...
TEST(eof_validation, EOF1_returncontract_valid)
{
    ...
    // initcontainer_index = 1
    cont = eof_bytecode(bytecode(0) + 0 + OP_RETURNCONTRACT + Opcode{1}, 2)
               .container(embedded)
               .container(embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::success);

    // initcontainer_index = 255
    cont = eof_bytecode(bytecode(0) + 0 + OP_RETURNCONTRACT + Opcode{255}, 2);
    for (auto i = 0; i < 256; ++i)
        cont.container(embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::success);
}
...
TEST(eof_validation, EOF1_returncontract_invalid)
{
    // truncated immediate
    const auto embedded = eof_bytecode(bytecode{OP_INVALID});
    auto cont = eof_bytecode(bytecode(0) + 0 + OP_RETURNCONTRACT, 4).container(embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::truncated_instruction);

    // referring to non-existent container section
    cont = eof_bytecode(bytecode(0) + 0 + OP_RETURNCONTRACT + Opcode{1}, 4).container(embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_container_section_index);
    cont = eof_bytecode(bytecode(0) + 0 + OP_RETURNCONTRACT + Opcode{0xff}, 4).container(embedded);
    EXPECT_EQ(validate_eof(cont, EVMC_PRAGUE), EOFValidationError::invalid_container_section_index);

    // Unreachable code after RETURNCONTRACT
    cont = eof_bytecode(bytecode(0) + 0 + OP_RETURNCONTRACT + Opcode{0} + OP_STOP, 2)
               .container(embedded);
