...
// TODO not possible with new CREATE3 design, should be changed to CREATE4 validation test
...
//    expect.post[*tx.to].nonce = pre.get(*tx.to).nonce + 1;  // CREATE caller's nonce must be
//    bumped expect.post[*tx.to].storage[0x01_bytes32] = 0x00_bytes32;  // CREATE must fail
